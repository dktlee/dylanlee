...
``` {r Data Analysis: Part 2 - Distribution, message=FALSE}
...
hist.totals.PosNum <- ggplot(data_player_totals, aes(x=PosNum,fill=AllNba,color=AllNba)) + 
...
hist.totals.PTS <- ggplot(data_player_totals, aes(x=PTS,fill=AllNba,color=AllNba)) + 
                        ...
                        ggtitle("Total Stats") + 
                        theme(plot.title = element_text(hjust = 0.5))
hist.pergame.PosNum <- ggplot(data_player_pergame, aes(x=PosNum,fill=AllNba,color=AllNba)) + 
                        geom_histogram(position = "identity", alpha = 0.5) +
...
hist.pergame.PTS <- ggplot(data_player_pergame, aes(x=PTS,fill=AllNba,color=AllNba)) + 
                        ...
                        ggtitle("Per Game Stats") + 
                        theme(plot.title = element_text(hjust = 0.5))
hist.adv.PosNum <- ggplot(data_player_adv, aes(x=PosNum,fill=AllNba,color=AllNba)) + 
                        geom_histogram(position = "identity", alpha = 0.5) +
...
# filtered datapoints based on Minutes Playued to be more restrictive, so considered players
# who have more than 1600 minutes played in a given season for the data_player_totals & 
# data_player adv datasets / more than 28 mintues per game for the data_player_pergame dataset
data_player_totals <- data_player_totals[which(data_player_totals$MP>=1600),]
data_player_pergame <- data_player_pergame[which(data_player_pergame$MP>=28),]
data_player_adv <- data_player_adv[which(data_player_adv$MP>=28),]
```
...
``` {r Data Analysis: Part 2 - Distribution}
# we see that we are working with quite a large data set (~8500-10500 rows by 32-35 columns)
# by looking at the distribution of our data, comparing All-NBA players against the rest of the leauge,
# we may find ways to filter out data points that could potentially dilute the model's ability to
# showcase elite players, but instead flood the model with bench/rotational players

# the plots are histograms counting the number of players (y-axis) who fall into the bucket determined
# by the stat on the x-axis, split between All-NBA players and non All-NBA players
ggpairs.totals <- ggpairs(data_player_totals, columns=c(4,5,9,11:35,7))
ggpairs.pergame <- ggpairs(data_player_pergame, columns=c(4,5,9,11:35,7))
ggpairs.adv <- ggpairs(data_player_adv, columns=c(4,5,9,11:32,7))
histo.totals <- lapply(1:ggpairs.totals$ncol, function(j) getPlot(ggpairs.totals,i=29,j=j))
histo.pergame <- lapply(1:ggpairs.pergame$ncol, function(j) getPlot(ggpairs.pergame,i=29,j=j))
histo.adv <- lapply(1:ggpairs.adv$ncol, function(j) getPlot(ggpairs.adv,i=26,j=j))
histo.totals
histo.pergame
histo.adv
...
# we go from having about 8500-10000 data points to about 5700-6200 data points
```
```{r Data Analysis- Part 1}
...
```{r Data Analysis: Part 3 - Correlation}
...
```{r Data Analysis- Part 4}
# lets look at indicators for Total Number of All-NBA Selections over the course of a player's career
corr.matrix.career <- cor(data_player_career[,c("AllNba","TeamWins","PosNum","Age","GP","GS","MP","FGM","FGA","FGperc","ThreePM",
                                                "ThreePA","ThreePperc","TwoPM","TwoPA","TwoPperc","eFGperc","FTM","FTA",
                                                "FTperc","ORB","DRB","TRB","AST","STL","BLK","TOV","PF","PTS")])
corrplot(corr.matrix.career[1,2:29,drop=FALSE], title="Correlation: Player Career", mar=c(0,0,1,0))

ggpairs.career <- ggpairs(data_player_career, columns = c(4,8,9,11:35,7), mapping=ggplot2::aes(colour = AllNba))
histo.career <- lapply(1:ggpairs.career$ncol, function(j) getPlot(ggpairs.career,i=29,j=j))
histo.career
...
``` {r Data Analysis- Part 5}
# look at the distribution of the stats we have 
cat("Player Totals Data:","\n")
...
cat("\nPlayer Advanced Stat Data:","\n")
# the summary statistics show a large range of values for any given statistic, as expected
# should normalize each statistic by season by dividing by the season's max, so that each
# statistic now lies between (0,1), and the stat leader being the max: 1
# this is because we don't want outlier's being too low or too high in a stat category
# hurting the model

# Player Stat Per Game
for (year in levels(as.factor(data_player_pergame$Season))) {
  for (index in c(5,9,11:35)) {
        data_year <- data_player_pergame[which(data_player_pergame$Season==year),]
        data_player_pergame[which(data_player_pergame$Season==year),][,index] <- data_year[,index]/max(data_year[,index])
        data_player_pergame[,index] <- round(data_player_pergame[,index], digits = 3)
  }
}

# Player Stat Total
for (year in levels(as.factor(data_player_totals$Season))) {
  for (index in c(5,9,11:35)) {
        data_year <- data_player_totals[which(data_player_totals$Season==year),]
        data_player_totals[which(data_player_totals$Season==year),][,index] <- data_year[,index]/max(data_year[,index])
        data_player_totals[,index] <- round(data_player_totals[,index], digits = 3)
  }
}

# Player Stat Advanced
for (year in levels(as.factor(data_player_adv$Season))) {
  for (index in c(5,9,11:32)) {
        data_year <- data_player_adv[which(data_player_adv$Season==year),]
        data_player_adv[which(data_player_adv$Season==year),][,index] <- data_year[,index]/max(data_year[,index])
        data_player_adv[,index] <- round(data_player_adv[,index], digits = 3)
  }
}
