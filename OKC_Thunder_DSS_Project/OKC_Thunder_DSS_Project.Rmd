...
# lm.guard.total.reduced <- step(lm.guard.total.full)
...
log.guard.total.reduced <- glm(TotalAllNba ~ TeamWins+GPFGM+FGA+FGperc+eFGperc+FTM+TRB+AST+STL+TOV+PF+PTS,
                               data = guard_data_totals[which(guard_data_totals$Season != 2019),], family = binomial(link="logit"))
...
log.guard.adv.reduced <- glm(TotalAllNba ~ TeamWins+GP+MP+TRBperc+ASTperc+USGperc+OWS+DWS+WS+WSper48,
                             data = guard_data_adv[which(guard_data_adv$Season != 2019),], family = binomial(link="logit"))
...
full_vs_red_acc <- data.frame(Data=c("Full Linear","Reduced Linear","Full Logistic","Reduced Linear"),
