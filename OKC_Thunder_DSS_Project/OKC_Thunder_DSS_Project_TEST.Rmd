...
forecast_next_year_stats <- function(last_year_data,two_years_ago_data,three_years_ago_data) {
  ...
  for (index in dim(last_year_data)[1]) {
    player <- last_year_data[index,]
    df.player <- last_year_data[0,]
    # stop forecasting players who have been in the league for 15 years or more 
    if (player$YearsInNba < 15) {
      if (player$Age <= 28) {
        age_adj <- (28 - player$Age)*0.004
      } else {
        age_adj <- (28 - player$Age)*0.002
      }
      # add ~5 game noise (assumption made that players will consitently play about the same amount as the season before)
      ...
      player_proj_gp <- round(min(82,player$GP + rnorm(1,0,5)),digits=0)
      player_proj_gs <- round(min(82,player$GS + rnorm(1,0,5)),digits=0)
      # All-NBA players play about 35 minutes per game
      player_proj_mp <- round(min(3936,player$MP + rnorm(1,0,5*35)),digits=0)
      if (player$Player %in% three_years_ago_data$Player) {
        ...
        league_avg_weighted_fg_miss <- 1000*(6*player_mp_last_year*(fg_miss_last_year/mp_last_year) +
                                         ...
                                         1*player_mp_three_years_ago*(fg_miss_three_years_ago/mp_three_years_ago))/weighted_minutes_played
        league_avg_weighted_threepm <- 1000*(6*player_mp_last_year*(threepm_last_year/mp_last_year) +
                                         ...
                                         1*player_mp_three_years_ago*(threepm_three_years_ago/mp_three_years_ago))/weighted_minutes_played
        league_avg_weighted_threep_miss <- 1000*(6*player_mp_last_year*(threep_miss_last_year/mp_last_year) +
                                         ...
                                         1*player_mp_three_years_ago*(threep_miss_three_years_ago/mp_three_years_ago))/weighted_minutes_played
        ...
        league_avg_weighted_ft_miss <- 1000*(6*player_mp_last_year*(ft_miss_last_year/mp_last_year) +
                                         ...
                                         1*player_mp_three_years_ago*(ft_miss_three_years_ago/mp_three_years_ago))/weighted_minutes_played
        ...
        player_proj_fgm <- (weighted_fgm+league_avg_weighted_fgm) / (player_mp_last_year+1000) * player_proj_mp * (1+age_adj)
        player_proj_fg_miss <- (weighted_fg_miss+league_avg_weighted_fg_miss) / (player_mp_last_year+1000) * player_proj_mp * (1-age_adj)
        player_proj_threepm <- (weighted_threepm+league_avg_weighted_threepm) / (player_mp_last_year+1000) * player_proj_mp * (1+age_adj)
        player_proj_threep_miss <- (weighted_threep_miss+league_avg_weighted_threep_miss) / (player_mp_last_year+1000) * player_proj_mp * (1-age_adj)
        player_proj_ftm <- (weighted_ftm+league_avg_weighted_ftm) / (player_mp_last_year+1000) * player_proj_mp * (1+age_adj)
        player_proj_ft_miss <- (weighted_ft_miss+league_avg_weighted_ft_miss) / (player_mp_last_year+1000) * player_proj_mp * (1-age_adj)
        player_proj_orb <- (weighted_orb+league_avg_weighted_orb) / (player_mp_last_year+1000) * player_proj_mp * (1+age_adj)
        player_proj_drb <- (weighted_fgm+league_avg_weighted_drb) / (player_mp_last_year+1000) * player_proj_mp * (1+age_adj)
        player_proj_trb <- player_proj_orb+player_proj_drb
        player_proj_ast <- (weighted_ast+league_avg_weighted_ast) / (player_mp_last_year+1000) * player_proj_mp * (1+age_adj)
        player_proj_stl <- (weighted_stl+league_avg_weighted_stl) / (player_mp_last_year+1000) * player_proj_mp * (1+age_adj)
        player_proj_blk <- (weighted_blk+league_avg_weighted_blk) / (player_mp_last_year+1000) * player_proj_mp * (1+age_adj)
        player_proj_tov <- (weighted_tov+league_avg_weighted_tov) / (player_mp_last_year+1000) * player_proj_mp * (1-age_adj)
        player_proj_pf <- (weighted_pf+league_avg_weighted_pf) / (player_mp_last_year+1000) * player_proj_mp * (1-age_adj)
        player_proj_fga <- player_proj_fgm+player_proj_fg_miss
        player_proj_fgperc <- player_proj_fgm/player_proj_fga
        player_proj_threepa <- player_proj_threepm+player_proj_threep_miss
        player_proj_threepperc <- player_proj_threepm/player_proj_threepa
        player_proj_fta <- player_proj_ftm+player_proj_ft_miss
        player_proj_ftperc <- player_proj_ftm/player_proj_fta
        player_proj_twopm <- player_proj_fgm-player_proj_threepm
        player_proj_twopa <- player_proj_fga-player_proj_threepa
        player_proj_twopperc <- player_proj_twopm/player_proj_twopa
        player_proj_efgperc <- (player_proj_fgm+0.5*player_proj_threepm)/player_proj_fga
        ...
        league_avg_weighted_fg_miss <- 1000*(6*player_mp_last_year*(fg_miss_last_year/mp_last_year) +
                                         3*player_mp_two_years_ago*(fg_miss_two_years_ago/mp_two_years_ago))/weighted_minutes_played
        league_avg_weighted_threepm <- 1000*(6*player_mp_last_year*(threepm_last_year/mp_last_year) +
                                         3*player_mp_two_years_ago*(threepm_two_years_ago/mp_two_years_ago))/weighted_minutes_played
        league_avg_weighted_threep_miss <- 1000*(6*player_mp_last_year*(threep_miss_last_year/mp_last_year) +
                                         3*player_mp_two_years_ago*(threep_miss_two_years_ago/mp_two_years_ago))/weighted_minutes_played
        league_avg_weighted_ftm <- 1000*(6*player_mp_last_year*(ftm_last_year/mp_last_year) +
                                         3*player_mp_two_years_ago*(ftm_two_years_ago/mp_two_years_ago))/weighted_minutes_played
        league_avg_weighted_ft_miss <- 1000*(6*player_mp_last_year*(ft_miss_last_year/mp_last_year) +
                                         3*player_mp_two_years_ago*(ft_miss_two_years_ago/mp_two_years_ago))/weighted_minutes_played
        league_avg_weighted_orb <- 1000*(6*player_mp_last_year*(orb_last_year/mp_last_year) +
        ...
        player_proj_fgm <- round((weighted_fgm+league_avg_weighted_fgm) / (weighted_minutes_played+1000) * player_proj_mp * (1+age_adj),digits=0)
        player_proj_fg_miss <- round((weighted_fg_miss+league_avg_weighted_fg_miss) / (weighted_minutes_played+1000) * player_proj_mp * (1-age_adj),digits=0)
        player_proj_threepm <- round((weighted_threepm+league_avg_weighted_threepm) / (weighted_minutes_played+1000) * player_proj_mp * (1+age_adj),digits=0)
        player_proj_threep_miss <- round((weighted_threep_miss+league_avg_weighted_threep_miss) / (weighted_minutes_played+1000) * player_proj_mp * (1-age_adj),digits=0)
        player_proj_ftm <- round((weighted_ftm+league_avg_weighted_ftm) / (weighted_minutes_played+1000) * player_proj_mp * (1+age_adj),digits=0)
        player_proj_ft_miss <- round((weighted_ft_miss+league_avg_weighted_ft_miss) / (weighted_minutes_played+1000) * player_proj_mp * (1-age_adj),digits=0)
        player_proj_orb <- round((weighted_orb+league_avg_weighted_orb) / (weighted_minutes_played+1000) * player_proj_mp * (1+age_adj),digits=0)
        player_proj_drb <- round((weighted_fgm+league_avg_weighted_drb) / (weighted_minutes_played+1000) * player_proj_mp * (1+age_adj),digits=0)
        player_proj_trb <- player_proj_orb+player_proj_drb
        player_proj_ast <- round((weighted_ast+league_avg_weighted_ast) / (weighted_minutes_played+1000) * player_proj_mp * (1+age_adj),digits=0)
        player_proj_stl <- round((weighted_stl+league_avg_weighted_stl) / (weighted_minutes_played+1000) * player_proj_mp * (1+age_adj),digits=0)
        player_proj_blk <- round((weighted_blk+league_avg_weighted_blk) / (weighted_minutes_played+1000) * player_proj_mp * (1+age_adj),digits=0)
        player_proj_tov <- round((weighted_tov+league_avg_weighted_tov) / (weighted_minutes_played+1000) * player_proj_mp * (1-age_adj),digits=0)
        player_proj_pf <- round((weighted_pf+league_avg_weighted_pf) / (weighted_minutes_played+1000) * player_proj_mp * (1-age_adj),digits=0)
        player_proj_fga <- player_proj_fgm+player_proj_fg_miss
        player_proj_fgperc <- round(player_proj_fgm/player_proj_fga,digits=3)
        player_proj_threepa <- player_proj_threepm+player_proj_threep_miss
        player_proj_threepperc <- round(player_proj_threepm/player_proj_threepa,digits=3)
        player_proj_fta <- player_proj_ftm+player_proj_ft_miss
        player_proj_ftperc <- round(player_proj_ftm/player_proj_fta,digits=3)
        player_proj_twopm <- player_proj_fgm-player_proj_threepm
        player_proj_twopa <- player_proj_fga-player_proj_threepa
        player_proj_twopperc <- round(player_proj_twopm/player_proj_twopa,3)
        player_proj_efgperc <- round((player_proj_fgm+0.5*player_proj_threepm)/player_proj_fga,digits=3)
      ...
      df.player <- data.frame(Player=player$Player,Rk=player$Rk,Pos=player$Pos,Tm=player$Tm,Season=player$Season+1,TotalAllNba=0,
                     Age=player$Age+1,GP=player_proj_gp,GS=player_proj_gs,MP=player_proj_mp,FGM=player_proj_fgm,FGA=player_proj_fga,
                     FGperc=player_proj_fgperc,ThreePM=player_proj_threepm,ThreePA=player_proj_threepa,ThreePperc=player_proj_threepperc,
                     TwoPM=player_proj_twopm,TwoPA=player_proj_twopa,TwoPperc=player_proj_twopperc,eFGperc=player_proj_efgperc,
                     FTM=player_proj_ftm,FTA=player_proj_fta,FTperc=player_proj_ftperc,ORB=player_proj_orb,DRB=player_proj_drb,
                     TRB=player_proj_trb,AST=player_proj_ast,STL=player_proj_stl,BLK=player_proj_blk,TOV=player_proj_tov,
                     PF=player_proj_pf,PTS=player_proj_pts,DraftYear=player$DraftYear,YearsInNba=player$YearsInNba+1)
    }
  }
...
##################################################################
################   Load Player Draft Year Data   #################
##################################################################
loadDraftYearData <- function() {
  # creating final data frame and setting variable types for the columns
  df <- data.frame(DraftYear=integer(),Player=character())
  df.temp <- read.csv("player_draft_year/player_draft_year.csv",header = TRUE,stringsAsFactors = FALSE)
  ...
  df <- data.frame(DraftYear=df.temp$Year,Player=df.temp$Player)
...
loadTotalsData <- function(yr_start, yr_end) {
  ...
  df <- df[, c(1, 2, 3, 5, 31, 32, 4, seq(6, 30)), ]
...
data_player_draft_year <- loadDraftYearData()
data_player_totals <- merge(data_player_totals,data_player_draft_year,by=c("Player"))
data_player_totals$YearsInNba <- data_player_totals$Season - data_player_totals$DraftYear
...
data_player_totals %>% select(Age,GP,GS,MP,FGM,FGA,FGperc,ThreePM,ThreePA,ThreePperc,
...
corr.matrix.totals <- cor(data_player_totals[,c("TotalAllNba","Age","GP","GS","MP","FGM","FGA","FGperc","ThreePM",
...
# Player Stat Total
data_player_totals_filtered <- normalize_stats(data_player_totals_filtered)
```
...
lm.guard.total.reduced <- lm(TotalAllNba ~ GP+GS+MP+FGM+FGperc+ThreePperc+TwoPM+TwoPA+TwoPperc+
...
lm.forward.total.reduced <- lm(TotalAllNba ~ GP+MP+FGperc+ThreePperc+TwoPM+TwoPA+
                                             eFGperc+FTM+FTperc+ORB+TRB+AST+STL+BLK+TOV+PF+PTS,
                               data = forward_data_totals[which(forward_data_totals$Season != 2019),])
...
lm.center.total.reduced <- lm(TotalAllNba ~ MP+Age+FGA+FGperc+ThreePM+ThreePA+TwoPM+TwoPA+TwoPperc+
                                            FTM+FTA+ORB+TRB+AST+STL+BLK+TOV,
                              data = center_data_totals[which(center_data_totals$Season != 2019),])
...
log.guard.total.reduced <- glm(TotalAllNba ~ GP+MP+FGM+FGA+ThreePM+ThreePA+TwoPperc+eFGperc++FTM+TRB+AST+STL+TOV+PF+PTS,
                               data = guard_data_totals[which(guard_data_totals$Season != 2019),], family = binomial(link="logit"))
...
log.forward.total.reduced <- glm(TotalAllNba ~ GP+GS+MP+ThreePM+ThreePA+ThreePperc+TwoPM+TwoPperc+eFGperc+FTM+DRB+AST+STL+BLK+TOV+PTS,
                                 data = forward_data_totals[which(forward_data_totals$Season != 2019),], family = binomial(link="logit"))
...
# center
log.center.total.full <- glm(TotalAllNba ~ GP+GS+MP+Age+FGM+FGA+FGperc+ThreePM+ThreePA+ThreePperc+TwoPM+TwoPA+TwoPperc+
                                        eFGperc+FTM+FTA+FTperc+ORB+DRB+TRB+AST+STL+BLK+TOV+PF+PTS,
...
full_vs_red_plot <- barplot(c(full_vs_red_acc$Linear[1],full_vs_red_acc$Linear[2],full_vs_red_acc$Logistic[1],full_vs_red_acc$Logistic[2]),
                            names.arg = full_vs_red_acc$Data,
                            ylim=c(0,1), las=2, col=c(rep(colours[1],2),rep(colours[2],2)),
...
text(x=full_vs_red_plot,
     ...
     label=round(c(full_vs_red_acc$Linear[1],full_vs_red_acc$Linear[2],full_vs_red_acc$Logistic[1],full_vs_red_acc$Logistic[2]),
               digits=3),
     pos=3, cex=0.7)
legend("bottomright",c("Total Stats"),
...
par(mfrow=c(1,1))
colours <- viridis(3)
full_vs_red_plot <- barplot(c(linear_vs_log$Linear[1],linear_vs_log$Logistic[1],
                            ...
                            ylim=c(0,1), las=2, col=c(rep(colours[1],2),rep(colours[2],2),rep(colours[3],2)),
                            ylab="Accuracy",main="Linear vs Logistic Model Accuracy - LOOCV", cex.names = 0.6)
text(x=full_vs_red_plot,
     y=c(linear_vs_log$Linear[1],linear_vs_log$Logistic[1],
         linear_vs_log$Linear[2],linear_vs_log$Logistic[2],
         linear_vs_log$Linear[3],linear_vs_log$Logistic[3]),
...
```
```{r Forecasting}
data_2019 <- data_player_totals[which(data_player_totals$Season == 2019 & data_player_totals$GP>=48),]
data_2018 <- data_player_totals[which(data_player_totals$Season == 2018 & data_player_totals$GP>=48),]
data_2017 <- data_player_totals[which(data_player_totals$Season == 2017 & data_player_totals$GP>=48),]
data_2020 <- forecast_next_year_stats(data_2019,data_2018,data_2017)
```
