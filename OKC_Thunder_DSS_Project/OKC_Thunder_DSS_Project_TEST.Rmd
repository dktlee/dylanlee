...
simulate_all_nba <- function(data) {
  ...
  for (year in 2020:2034) {
    ...
    full_model_center <- glm(TotalAllNba ~ GP+GS+MP+Age+FGM+FGA+FGperc+ThreePM+ThreePA+ThreePperc+TwoPM+TwoPA+TwoPperc+
                                          eFGperc+FTM+FTA+FTperc+ORB+DRB+TRB+AST+STL+BLK+TOV+PF+PTS,
    ...
    all_nba_guard <- test_guard_set[which(pred_guard >= pred_guard[order(pred_guard,decreasing=TRUE)][6]),]
    ...
    all_nba_center <- test_center_set[which(pred_center >= pred_center[order(pred_center,decreasing=TRUE)][3]),]
...
forecast_next_year_stats <- function(last_year_data,two_years_ago_data,three_years_ago_data) {
  ...
  for (index in 1:dim(last_year_data)[1]) {
    ...
    if (player$YearsInNba < 17 & player$PTS > 0) {
      ...
      # add ~4 game noise (assumption made that players will consitently play about the same amount as the season before)
      ...
      player_proj_gp <- max(0,round(min(82,player$GP + rnorm(1,0,4)),digits=0))
      player_proj_gs <- max(0,round(min(player_proj_gp,player$GS + rnorm(1,0,4)),digits=0))
      # All-NBA players play about 35 minutes per game
      player_proj_mp <- max(0,round(min(player_proj_gp*48,player$MP + rnorm(1,0,4*35)),digits=0))
...
data_player_totals_filtered <- data_player_totals[which(data_player_totals$GP>=48),c(1:34)]
```
...
seeds <- sample(1:10000,10000)
luka_most <- 0
kyrie_most <- 0
steph_most <- 0
kat_most <- 0
sim_results <- data.frame(Seed=int(),Luka=int(),Kyrie=int(),Steph=int(),Kat=int())
for (i in seeds) {
  set.seed(i)
  df.forecast <- simulate_all_nba(data_player_totals_filtered)
  luka <- dim(df.forecast[which(df.forecast$Player=="Luka Dončić"&df.forecast$TotalAllNba==1&df.forecast$Season>=2020),])[1]
  ...
  rbind(sim_results,data.frame(Seed=i,Luka=luka,Kyrie=kyrie,Steph=steph,Kat=kat))
}
luka_likelihood <- luka_most / 10000
kyrie_likelihood <- kyrie_most / 10000
steph_likelihood <- steph_most / 10000
kat_likelihood <- kat_most / 10000
