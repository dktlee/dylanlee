...
##################################################################
# conduct leave-one-out cross-validation by training the model on all the season's
# data, except one season, and leaving that one season out to test the model on
...
normalize_stats <- function (data) {
  for (year in levels(as.factor(data$Season))) {
    for (index in c(8:33)) {
      ...
      data[which(data$Season == year), ][, index] <- data_year[, index] /
        max(data_year[, index])
...
##################################################################
# this function will conduct the forecast of next year's stats, add the past season's
# stats to the training data set, and then predict All-NBA players for the forecasted
# season. The model is updated with each iteration when a new All-NBA team is selected
simulate_all_nba <- function(data) {
  df.forecast <- data
  for (year in 2020:2034) {
    df.temp <- forecast_next_year_stats(
      df.forecast[which(df.forecast$Season == (year - 1)), ],
...
##################################################################
# a Simple Projection System, similar to the algorithm used by Basketball-Reference.com 
# to project a player's stats for the next season modified here to project a player's
# Total stats for the season. Not the most sophisticated projection system, 
# but it at least generates comparable stats that may be used
forecast_next_year_stats <- function(last_year_data,two_years_ago_data,three_years_ago_data) {
  ...
  for (index in 1:dim(last_year_data)[1]) {
    ...
    if (player$YearsInNba < 17 & player$PTS > 0) {
      ...
      # add ~4 game variance noise (assumption made that players will consitently play
      # about the same amount as the season before)
      # unfortunate for players who were injured in one of the prior years because
      # their projections will be based off the time they weren't injured and could play.
      # However, this does capture the effect of a players durability year-to-year
      # so I am fine with making this assumption
      player_proj_gp <- max(0, round(min(82, player$GP + rnorm(1, 0, 2)), digits = 0))
      player_proj_gs <- max(0, round(min(player_proj_gp, player$GS + rnorm(1, 0, 2)),
                                     digits = 0))
      ...
      if (player$Player %in% three_years_ago_data$Player &
        ...
        player_mp_last_year <- last_year_data[which(last_year_data$Player ==
                                                      player$Player), ]$MP
        player_mp_two_years_ago <- two_years_ago_data[which(two_years_ago_data$Player ==
                                                              player$Player), ]$MP
        ...
        player_mp_last_year <- last_year_data[which(last_year_data$Player ==
                                                      player$Player), ]$MP
        player_mp_three_years_ago <- three_years_ago_data[which(
          three_years_ago_data$Player == player$Player), ]$MP
        # last season weight = 6, two seasons ago weight = 3, three seasons ago weight = 1
        ...
        weighted_fgm <- 1 * three_years_ago_data[which(three_years_ago_data$Player ==
                                                         player$Player), ]$FGM
        ...
        weighted_drb <- 1 * three_years_ago_data[which(three_years_ago_data$Player ==
                                                         player$Player), ]$DRB
        weighted_ast <- 1 * three_years_ago_data[which(three_years_ago_data$Player ==
                                                         player$Player), ]$AST
        weighted_stl <- 1 * three_years_ago_data[which(three_years_ago_data$Player ==
                                                         player$Player), ]$STL
        weighted_blk <- 1 * three_years_ago_data[which(three_years_ago_data$Player ==
                                                         player$Player), ]$BLK
        weighted_tov <- 1 * three_years_ago_data[which(three_years_ago_data$Player ==
                                                         player$Player), ]$TOV
        weighted_pf <- 1 * three_years_ago_data[which(three_years_ago_data$Player ==
                                                        player$Player), ]$PF
        # scaled to 1000 minutes
      ...
      df.player <- data.frame(Player = player$Player,Rk = player$Rk,Pos = player$Pos,
                              Tm = player$Tm,Season = player$Season + 1,TotalAllNba = 0,
                              Age = player$Age + 1,GP = player_proj_gp,
                              GS = player_proj_gs,MP = player_proj_mp,
                              FGM = player_proj_fgm,FGA = player_proj_fga,
                              FGperc = player_proj_fgperc,
                              ThreePM = player_proj_threepm,ThreePA = player_proj_threepa,
                              ThreePperc = player_proj_threepperc,
                              TwoPM = player_proj_twopm,TwoPA = player_proj_twopa,
                              TwoPperc = player_proj_twopperc,
                              eFGperc = player_proj_efgperc,FTM = player_proj_ftm,
                              FTA = player_proj_fta,FTperc = player_proj_ftperc,
                              ORB = player_proj_orb,DRB = player_proj_drb,
                              TRB = player_proj_trb,AST = player_proj_ast,
                              STL = player_proj_stl,BLK = player_proj_blk,
                              TOV = player_proj_tov,PF = player_proj_pf,
                              PTS = player_proj_pts,
                              DraftYear = player$DraftYear,
                              YearsInNba = player$YearsInNba + 1)
...
loadAwardsData <- function() {
  ...
  # add All-NBA flag to identify players when joined together with larger 
...
loadTotalsData <- function(yr_start, yr_end) {
  ...
  df <- data.frame(Rk = integer(),Player = character(),Pos = character(),Age = double(),
                   Tm = character(),GP = double(),GS = double(),MP = double(),
                   FGM = double(),FGA = double(),FGperc = double(),
                   ThreePM = double(),ThreePA = double(),ThreePperc = double(),
                   TwoPM = double(),TwoPA = double(),TwoPperc = double(),
