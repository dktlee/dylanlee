...
test_all_nba <- loadAwardsData()
test_player_totals <- loadTotalsData(2000, YEAR_END)
test_player_totals$YearsInNba <- test_player_totals$Season -
                      as.integer(test_player_totals$DraftYear)

test_player_totals$AllNba <- FALSE
test_player_totals$AllNba <- ifelse(test_player_totals$TotalAllNba == 1, TRUE, FALSE)

head(test_all_nba[sample(nrow(test_all_nba), 10),], 10)
head(test_player_totals[sample(nrow(test_player_totals), 10),], 10)
```
