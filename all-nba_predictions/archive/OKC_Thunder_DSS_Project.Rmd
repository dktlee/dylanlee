...
# use par(mfrow=c(2,2)) to graph plots in grid: https://www.statmethods.net/advgraphs/layout.html
```
...
```{r Modelling, waring = FALSE, message = FALSE}
...
lm.guard.total.reduced <- lm(TotalAllNba ~ TeamWins+GP+GS+MP+FGM+FGA+FGperc+TwoPM+TwoPA+TwoPperc+
                                          eFGperc+FTM+FTperc+ORB+TRB+AST+STL+BLK+TOV+PF,
                             data = guard_data_totals[which(guard_data_totals$Season != 2019),])
...
lm.forward.total.reduced <- lm(TotalAllNba ~ TeamWins+GP+GS+MP+FGperc+TwoPM+TwoPA+TwoPperc+
                                             eFGperc+FTM+FTperc+ORB+DRB+AST+STL+BLK+TOV+PTS,
                               data = forward_data_totals[which(forward_data_totals$Season != 2019),])
...
lm.center.total.reduced <- lm(TotalAllNba ~ TeamWins+Age+MP+FGA+FGperc+ThreePA+TwoPM+TwoPA+
                                            FTA+ORB+TRB+BLK+TOV+PF,
                              data = center_data_totals[which(center_data_totals$Season != 2019),])
...
log.guard.total.reduced <- glm(TotalAllNba ~ TeamWins+GP+FGM+FGA+FGperc+eFGperc+FTM+TRB+AST+STL+TOV+PF+PTS,
                               data = guard_data_totals[which(guard_data_totals$Season != 2019),], family = binomial(link="logit"))
...
log.forward.total.reduced <- glm(TotalAllNba ~ TeamWins+Age+GP+FGperc+ThreePperc+TwoPM+eFGperc+FTA+FTperc+DRB+AST+BLK+TOV+PF+PTS,
                                 data = forward_data_totals[which(forward_data_totals$Season != 2019),], family = binomial(link="logit"))
...
log.center.total.reduced <- glm(TotalAllNba ~ TeamWins+GP+MP+FGA+FGperc+ThreePA+TwoPA+TwoPperc+FTA+ORB+DRB+AST+BLK+TOV,
                            data = center_data_totals[which(center_data_totals$Season != 2019),], family = binomial(link="logit"))
...
# lm.guard.pergame.reduced <- step(lm.guard.pergame.full, trace = 0)
lm.guard.pergame.reduced <- lm(TotalAllNba ~ TeamWins+GS+MP+FGM+FGA+FGperc+ThreePA+TwoPM+TwoPA+TwoPperc+
                                             eFGperc+FTM+FTA+FTperc+ORB+TRB+AST+STL+BLK+TOV+PF+PTS,
                               data = guard_data_pergame[which(guard_data_pergame$Season != 2019),])
...
# lm.forward.pergame.reduced <- step(lm.forward.pergame.full, trace = 0)
lm.forward.pergame.reduced <- lm(TotalAllNba ~ TeamWins+Age+GP+GS+MP+FGM+FGperc+ThreePM+ThreePA+ThreePperc+TwoPM+TwoPA+
                                        eFGperc+FTperc+ORB+TRB+AST+STL+BLK+TOV+PTS,
                                 data = forward_data_pergame[which(forward_data_pergame$Season != 2019),])
...
# lm.center.pergame.reduced <- step(lm.center.pergame.full, trace = 0)
lm.center.pergame.reduced <- lm(TotalAllNba ~ TeamWins+MP+FGM+FGperc+ThreePA+TwoPM+TwoPA+
                                        FTM+FTA+ORB+DRB+TRB+BLK+TOV+PF+PTS,
                                data = center_data_pergame[which(center_data_pergame$Season != 2019),])
...
# log.guard.pergame.reduced <- step(log.guard.pergame.full, trace = 0)
log.guard.pergame.reduced <- glm(TotalAllNba ~ TeamWins+GP+FGM+FGA+TwoPM+TwoPA+ORB+DRB+TRB+AST+STL+TOV+PF+PTS,
                                 data = guard_data_pergame[which(guard_data_pergame$Season != 2019),], family = binomial(link="logit"))
...
# log.forward.pergame.reduced <- step(log.forward.pergame.full, trace = 0)
log.forward.pergame.reduced <- glm(TotalAllNba ~ TeamWins+Age+GP+FGA+FGperc+ThreePperc+FTM+FTA+FTperc+DRB+AST+BLK+TOV+PTS,
                                   data = forward_data_pergame[which(forward_data_pergame$Season != 2019),], family = binomial(link="logit"))
...
# log.center.pergame.reduced <- step(log.center.pergame.full, trace = 0)
log.center.pergame.reduced <- glm(TotalAllNba ~ TeamWins+Age+GS+MP+FGM+ThreePA+TwoPperc+FTM+FTperc+TRB+AST+BLK,
                                  data = center_data_pergame[which(center_data_pergame$Season != 2019),], family = binomial(link="logit"))
...
# lm.guard.adv.reduced <- step(lm.guard.adv.full, trace = 0)
lm.guard.adv.reduced <- lm(TotalAllNba ~ TeamWins+MP+PER+TSperc+ThreePAr+ORBperc+DRBperc+TRBperc+
                                         STLperc+USGperc+OWS+DWS+WSper48+OBPM+DBPM+VORP,
                           data = guard_data_adv[which(guard_data_adv$Season != 2019),])
...
# lm.forward.adv.reduced <- step(lm.forward.adv.full, trace = 0)
lm.forward.adv.reduced <- lm(TotalAllNba ~ GP+MP+PER+TSperc+ThreePAr+FTr+ORBperc+ASTperc+STLperc+USGperc+OWS+DWS+WS+WSper48+DBPM,
                             data = forward_data_adv[which(forward_data_adv$Season != 2019),])
...
# lm.center.adv.reduced <- step(lm.center.adv.full, trace = 0)
lm.center.adv.reduced <- lm(TotalAllNba ~ Age+MP+TSperc+ThreePAr+FTr+ORBperc+DRBperc+TRBperc+
                                          ASTperc+STLperc+USGperc+OWS+DWS+WS+WSper48+OBPM+VORP,
                            data = center_data_adv[which(center_data_adv$Season != 2019),])
...
# log.guard.adv.reduced <- step(log.guard.adv.full, trace = 0)
log.guard.adv.reduced <- glm(TotalAllNba ~ TeamWins+GP+MP+TRBperc+ASTperc+USGperc+OWS+DWS+WSper48,
                             data = guard_data_adv[which(guard_data_adv$Season != 2019),], family = binomial(link="logit"))
...
# log.forward.adv.reduced <- step(log.forward.adv.full, trace = 0)
log.forward.adv.reduced <- glm(TotalAllNba ~ TeamWins+GP+MP+ThreePAr+ORBperc+ASTperc+USGperc+OWS+DWS+WSper48+OBPM+DBPM+VORP,
                               data = forward_data_adv[which(forward_data_adv$Season != 2019),], family = binomial(link="logit"))
...
# log.center.adv.reduced <- step(log.center.adv.full, trace = 0)
log.center.adv.reduced <- glm(TotalAllNba ~ Age+GP+MP+FTr+ORBperc+DRBperc+TRBperc+ASTperc+USGperc+OWS+DWS+DBPM,
                              data = center_data_adv[which(center_data_adv$Season != 2019),], family = binomial(link="logit"))
...
# lm.guard.merge.reduced <- step(lm.guard.merge.full, trace = 0)
lm.guard.merge.reduced <- lm(TotalAllNba ~ TeamWins+MP+FGA+ThreePM+ThreePA+TwoPM+TwoPperc+eFGperc+FTA+ORB+DRB+AST+
                                           PTS+PER+TSperc+FTr+DRBperc+ASTperc+STLperc+TOVperc+USGperc+OWS+WS+WSper48+DBPM,
                          data = guard_data_merge[which(guard_data_merge$Season != 2019),])
...
# lm.forward.merge.reduced <- step(lm.forward.merge.full, trace = 0)
lm.forward.merge.reduced <- lm(TotalAllNba ~ GP+MP+FGA+ThreePM+ThreePA+TwoPM+TwoPA+eFGperc+FTM+FTA+ORB+DRB+AST+BLK+PF+
                                             PTS+PER+TSperc+ThreePAr+ORBperc+DRBperc+ASTperc+STLperc+
                                             BLKperc+TOVperc+USGperc+DWS+WS+WSper48+DBPM+VORP,
                               data = forward_data_merge[which(forward_data_merge$Season != 2019),])
...
# lm.center.merge.reduced <- step(lm.center.merge.full, trace = 0)
lm.center.merge.reduced <- lm(TotalAllNba ~ MP+FGA+ThreePA+TwoPA+TwoPperc+FTA+FTperc+ORB+TRB+AST+
                                            PER+TSperc+FTr+DRBperc+STLperc+BLKperc+USGperc+DWS+WS+WSper48+OBPM+DBPM+BPM+VORP,
                          data = center_data_merge[which(center_data_merge$Season != 2019),])
...
# log.guard.merge.reduced <- step(log.guard.merge.full, trace = 0)
log.guard.merge.reduced <- glm(TotalAllNba ~ TeamWins+GP+FGM+FGA+FGperc+ThreePperc+TwoPM+
                                             TwoPA+FTperc+DRB+TRB+AST+PF+PTS+TSperc+ThreePAr+FTr+ORBperc+
                                             DRBperc+TRBperc+ASTperc+STLperc+USGperc+OWS+DWS+WSper48+DBPM+BPM,
                            data = guard_data_merge[which(guard_data_merge$Season != 2019),], family = binomial(link="logit"))
...
# log.forward.merge.reduced <- step(log.forward.merge.full, trace = 0)
log.forward.merge.reduced <- glm(TotalAllNba ~ TeamWins+GP+MP+FGM+FGA+FGperc+ThreePM+ThreePA+ThreePperc+
                                               TwoPA+eFGperc+FTM+FTA+ORB+DRB+TRB+STL+BLK+PF+PER+ThreePAr+
                                               FTr+ORBperc+STLperc+TOVperc+WS+WSper48+OBPM+DBPM+VORP,
                            data = forward_data_merge[which(forward_data_merge$Season != 2019),], family = binomial(link="logit"))
...
# log.center.merge.reduced <- step(log.center.merge.full, trace = 0)
log.center.merge.reduced <- glm(TotalAllNba ~ TeamWins+GP+FGM+FGperc+TwoPperc+FTM+FTA+BLK+PTS+TSperc+ThreePAr+
                                              ORBperc+DRBperc+TRBperc+DWS+OBPM+BPM+VORP,
                            data = center_data_merge[which(center_data_merge$Season != 2019),], family = binomial(link="logit"))
...
full_vs_red_acc <- data.frame(Data=c("Full Linear","Reduced Linear","Full Logistic","Reduced Logistic"),
                              Linear=c(lm.total.full.acc,lm.total.red.acc,lm.pergame.full.acc,lm.pergame.red.acc,
                                       lm.adv.full.acc,lm.adv.red.acc,lm.merge.full.acc,lm.merge.red.acc),
                              Logistic=c(log.total.full.acc,log.total.red.acc,log.pergame.full.acc,log.pergame.red.acc,
                                         log.adv.full.acc,log.adv.red.acc,log.merge.full.acc,log.merge.red.acc))
par(mfrow=c(1,1))
colours <- viridis(4)
full_vs_red_plot <- barplot(c(full_vs_red_acc$Linear[1],full_vs_red_acc$Linear[2],full_vs_red_acc$Logistic[1],full_vs_red_acc$Logistic[2],
                              full_vs_red_acc$Linear[3],full_vs_red_acc$Linear[4],full_vs_red_acc$Logistic[3],full_vs_red_acc$Logistic[4],
                              full_vs_red_acc$Linear[5],full_vs_red_acc$Linear[6],full_vs_red_acc$Logistic[5],full_vs_red_acc$Logistic[6],
                              full_vs_red_acc$Linear[7],full_vs_red_acc$Linear[8],full_vs_red_acc$Logistic[7],full_vs_red_acc$Logistic[8]),
                            names.arg = rep(full_vs_red_acc$Data,2),
                            ylim=c(0,1), las=2, col=c(rep(colours[1],4),rep(colours[2],4),rep(colours[3],4),rep(colours[4],4)),
                            ylab="Accuracy",main="Full vs Reduced Model Accuracy - 2019 Predictions", cex.names = 0.6)
text(x=full_vs_red_plot,
     y=c(full_vs_red_acc$Linear[1],full_vs_red_acc$Linear[2],full_vs_red_acc$Logistic[1],full_vs_red_acc$Logistic[2],
                              full_vs_red_acc$Linear[3],full_vs_red_acc$Linear[4],full_vs_red_acc$Logistic[3],full_vs_red_acc$Logistic[4],
                              full_vs_red_acc$Linear[5],full_vs_red_acc$Linear[6],full_vs_red_acc$Logistic[5],full_vs_red_acc$Logistic[6],
                              full_vs_red_acc$Linear[7],full_vs_red_acc$Linear[8],full_vs_red_acc$Logistic[7],full_vs_red_acc$Logistic[8]),
     label=round(c(full_vs_red_acc$Linear[1],full_vs_red_acc$Linear[2],full_vs_red_acc$Logistic[1],full_vs_red_acc$Logistic[2],
                              full_vs_red_acc$Linear[3],full_vs_red_acc$Linear[4],full_vs_red_acc$Logistic[3],full_vs_red_acc$Logistic[4],
                              full_vs_red_acc$Linear[5],full_vs_red_acc$Linear[6],full_vs_red_acc$Logistic[5],full_vs_red_acc$Logistic[6],
                              full_vs_red_acc$Linear[7],full_vs_red_acc$Linear[8],full_vs_red_acc$Logistic[7],full_vs_red_acc$Logistic[8]),
               digits=3),
     pos=3, cex=0.7)
legend("bottomright",c("Total Stats","Per Game Stats","Advanced Stats","Total & Adv Stats"),
      fill = colours, cex=0.8)
# see that the reduced models with less features perform the same as the full models 
# will use the reduced models going forward with cross validation to see which model performs the best
```
...
lm.guard.pergame.accuracies <- mean(cross_validation_all_nba_players(guard_data_pergame,lm.guard.pergame.reduced,"guard"))
lm.forward.pergame.accuracies <- mean(cross_validation_all_nba_players(forward_data_pergame,lm.forward.pergame.reduced,"forward"))
lm.center.pergame.accuracies <- mean(cross_validation_all_nba_players(center_data_pergame,lm.center.pergame.reduced,"center"))
log.guard.pergame.accuracies <- mean(cross_validation_all_nba_players(guard_data_pergame,log.guard.pergame.reduced,"guard"))
log.forward.pergame.accuracies <- mean(cross_validation_all_nba_players(forward_data_pergame,log.forward.pergame.reduced,"forward"))
log.center.pergame.accuracies <- mean(cross_validation_all_nba_players(center_data_pergame,log.center.pergame.reduced,"center"))

lm.guard.adv.accuracies <- mean(cross_validation_all_nba_players(guard_data_adv,lm.guard.adv.reduced,"guard"))
lm.forward.adv.accuracies <- mean(cross_validation_all_nba_players(forward_data_adv,lm.forward.adv.reduced,"forward"))
lm.center.adv.accuracies <- mean(cross_validation_all_nba_players(center_data_adv,lm.center.adv.reduced,"center"))
log.guard.adv.accuracies <- mean(cross_validation_all_nba_players(guard_data_adv,log.guard.adv.reduced,"guard"))
log.forward.adv.accuracies <- mean(cross_validation_all_nba_players(forward_data_adv,log.forward.adv.reduced,"forward"))
log.center.adv.accuracies <- mean(cross_validation_all_nba_players(center_data_adv,log.center.adv.reduced,"center"))

lm.guard.merge.accuracies <- mean(cross_validation_all_nba_players(guard_data_merge,lm.guard.merge.reduced,"guard"))
lm.forward.merge.accuracies <- mean(cross_validation_all_nba_players(forward_data_merge,lm.forward.merge.reduced,"forward"))
lm.center.merge.accuracies <- mean(cross_validation_all_nba_players(center_data_merge,lm.center.merge.reduced,"center"))
log.guard.merge.accuracies <- mean(cross_validation_all_nba_players(guard_data_merge,log.guard.merge.reduced,"guard"))
log.forward.merge.accuracies <- mean(cross_validation_all_nba_players(forward_data_merge,log.forward.merge.reduced,"forward"))
log.center.merge.accuracies <- mean(cross_validation_all_nba_players(center_data_merge,log.center.merge.reduced,"center"))

linear_vs_log <- data.frame(Data=c("LM - Total","LM - Total","LM - Total","LM - Pergame","LM - Pergame","LM - Pergame",
                                   "LM - Adv","LM - Adv","LM - Adv","LM - Merge","LM - Merge","LM - Merge"),
                           Linear=c(lm.guard.total.accuracies,lm.forward.total.accuracies,lm.center.total.accuracies,
                                    lm.guard.pergame.accuracies,lm.forward.pergame.accuracies,lm.center.pergame.accuracies,
                                    lm.guard.adv.accuracies,lm.forward.adv.accuracies,lm.center.adv.accuracies,
                                    lm.guard.merge.accuracies,lm.forward.merge.accuracies,lm.center.merge.accuracies),
                           Logistic=c(log.guard.total.accuracies,log.forward.total.accuracies,log.center.total.accuracies,
                                      log.guard.pergame.accuracies,log.forward.pergame.accuracies,log.center.pergame.accuracies,
                                      log.guard.adv.accuracies,log.forward.adv.accuracies,log.center.adv.accuracies,
                                      log.guard.merge.accuracies,log.forward.merge.accuracies,log.center.merge.accuracies))
par(mfrow=c(1,1))
colours <- viridis(3)
full_vs_red_plot <- barplot(c(linear_vs_log$Linear[1],linear_vs_log$Logistic[1],linear_vs_log$Linear[4],linear_vs_log$Logistic[4],
                              linear_vs_log$Linear[7],linear_vs_log$Logistic[7],linear_vs_log$Linear[10],linear_vs_log$Logistic[10],
                              linear_vs_log$Linear[2],linear_vs_log$Logistic[2],linear_vs_log$Linear[5],linear_vs_log$Logistic[5],
                              linear_vs_log$Linear[8],linear_vs_log$Logistic[8],linear_vs_log$Linear[11],linear_vs_log$Logistic[11],
                              linear_vs_log$Linear[3],linear_vs_log$Logistic[3],linear_vs_log$Linear[6],linear_vs_log$Logistic[6],
                              linear_vs_log$Linear[9],linear_vs_log$Logistic[9],linear_vs_log$Linear[12],linear_vs_log$Logistic[12]),
                            names.arg = rep(c("LM - Total","Log - Total","LM - Pergame","Log - Pergame",
                                              "LM - Adv","Log - Adv","LM - Merge","Log - Merge"),3),
                            ylim=c(0,1), las=2, col=c(rep(colours[1],8),rep(colours[2],8),rep(colours[3],8)),
                            ylab="Accuracy",main="Linear vs Logistic Model Accuracy - LOOCV", cex.names = 0.6)
text(x=full_vs_red_plot,
     y=c(linear_vs_log$Linear[1],linear_vs_log$Logistic[1],linear_vs_log$Linear[4],linear_vs_log$Logistic[4],
                              linear_vs_log$Linear[7],linear_vs_log$Logistic[7],linear_vs_log$Linear[10],linear_vs_log$Logistic[10],
                              linear_vs_log$Linear[2],linear_vs_log$Logistic[2],linear_vs_log$Linear[5],linear_vs_log$Logistic[5],
                              linear_vs_log$Linear[8],linear_vs_log$Logistic[8],linear_vs_log$Linear[11],linear_vs_log$Logistic[11],
                              linear_vs_log$Linear[3],linear_vs_log$Logistic[3],linear_vs_log$Linear[6],linear_vs_log$Logistic[6],
                              linear_vs_log$Linear[9],linear_vs_log$Logistic[9],linear_vs_log$Linear[12],linear_vs_log$Logistic[12]),
     label=round(c(linear_vs_log$Linear[1],linear_vs_log$Logistic[1],linear_vs_log$Linear[4],linear_vs_log$Logistic[4],
                              linear_vs_log$Linear[7],linear_vs_log$Logistic[7],linear_vs_log$Linear[10],linear_vs_log$Logistic[10],
                              linear_vs_log$Linear[2],linear_vs_log$Logistic[2],linear_vs_log$Linear[5],linear_vs_log$Logistic[5],
                              linear_vs_log$Linear[8],linear_vs_log$Logistic[8],linear_vs_log$Linear[11],linear_vs_log$Logistic[11],
                              linear_vs_log$Linear[3],linear_vs_log$Logistic[3],linear_vs_log$Linear[6],linear_vs_log$Logistic[6],
               ...
               digits=3),
     pos=3, cex=0.7)
legend("bottomright",c("Guards","Forwards","Centers"),
      fill = colours, cex=0.8)
# see that that the models perform differently depending on the position
# for guards, the Logistic model with Total Stats data and the Logistic model with Merged Stats data perform the best,
# accurately predicting the All-NBA guards 85% of the time, on average, from 2000-2019. I'll choose to use the Logistic
# model with total stats to stick with because its the simpler model but still performing the best
# for forwards, the Logistic model with Pergame Stats data and the Logistic model with Advanced Stats perform the best,
# accuratelty predicting the All-NBA forwards 82.5% of the time, on average, from 2000-2019. I'll choose to use the Logistic
# model with advanced stats to stick with because its the simpler model but still performing the best
# for centers, the Logistic model with Total Stats data and the Logistic model with Advanced Stats perform the best,
# accurately predicting the All-NBA centers 81.7% of the time, on average, from 2000-2019. I'll choose to use the Logistic
# model with advanced stats to stick with because its the simpler model but still performing the best
```
...
guard_data_total_2019 <- guard_data_totals[which(guard_data_totals$Season == 2019),]
center_data_adv_2019 <- center_data_adv[which(center_data_adv$Season == 2019),]

guard_2019_pred <- predict(log.guard.total.reduced,newdata = guard_data_total_2019,type = "response")
center_2019_pred <- predict(log.center.adv.reduced,newdata = center_data_adv_2019,type = "response")

# I should scale the predictions from the model because the the top 6 guards/forwards & top 3 centers 
# should have a probability of 1, so I'll divide the predictions by the 6th highest predication
luka_doncic_pred <- min(1, guard_2019_pred[which(guard_data_total_2019$Player=="Luka Dončić")] / 
                           guard_2019_pred[order(guard_2019_pred,decreasing=TRUE)][6])
kyrie_irving_pred <- min(1, guard_2019_pred[which(guard_data_total_2019$Player=="Kyrie Irving")] / 
                            guard_2019_pred[order(guard_2019_pred,decreasing=TRUE)][6])
steph_curry_pred <- min(1, guard_2019_pred[which(guard_data_total_2019$Player=="Stephen Curry")] / 
                           guard_2019_pred[order(guard_2019_pred,decreasing=TRUE)][6])
kat_pred <- min(1, center_2019_pred[which(center_data_adv_2019$Player=="Karl-Anthony Towns")] / 
                   center_2019_pred[order(center_2019_pred,decreasing=TRUE)][3])

luka_doncic_pred <- predict(log.guard.total.reduced,newdata=luka_doncic,type="response")
kyrie_irving_pred <- predict(log.guard.total.reduced,newdata=kyrie_irving,type="response")
steph_curry_pred <- predict(log.guard.total.reduced,newdata=steph_curry,type="response")
kat_pred <- predict(log.center.adv.reduced,newdata = kat,type="response")

all_nba_career_years <- read.csv("all_nba_awards/all_nba_career_years.csv", header = TRUE, stringsAsFactors = FALSE)
retired_all_nba <- all_nba_career_years[which(all_nba_career_years$Retired =="Y"),]
avg_career_length <- round(mean(retired_all_nba$Years.in.NBA, didgits=0))

# years in NBA: (Doncic = 1, Kyrie = 8, Steph = 10, Kat = 4)
luka_doncic_years <- avg_career_length - 1
kyrie_irving_years <- avg_career_length - 8
steph_curry_years <- avg_career_length - 10
kat_years <- avg_career_length - 4

# Total number of All-NBA selections remaining in each of player's career
luka <- luka_doncic_years*luka_doncic_pred
kyrie <- kyrie_irving_years*kyrie_irving_pred
steph <- steph_curry_years*steph_curry_pred
kat <- kat_years*kat_pred
